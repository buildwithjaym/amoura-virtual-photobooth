...
// --- Static Data moved outside component to prevent re-allocation on re-render ---
...
const THEMES = [
  { name: "Noir Date", text: "Cinematic black and red for intimate moments." },
  { name: "Soft Romance", text: "Warm pink tones for sweet memories." },
  { name: "Vintage Film", text: "Nostalgic grain and timeless photostrip texture." },
  { name: "Barkada Fun", text: "Playful frames for friends and celebrations." },
];

const REASONS = [
  { icon: Heart, title: "Emotion-first, not filter-first", text: "Amoura is designed around how a moment feels — closeness, excitement, nostalgia, and the joy of having something worth keeping." },
  { icon: Camera, title: "Feels like a real booth", text: "Countdowns, poses, photo sets, and instant strips make the experience feel intentional — not just another selfie." },
  { icon: Users, title: "Made for distance", text: "Dual Mode is built for couples and friends who want to capture the same moment even when they are not in the same place." },
  { icon: Download, title: "Designed to be saved", text: "Every session ends with a polished photostrip that feels ready to post, send, print, or keep in your memories." },
];

const STEPS = [
  { number: "01", title: "Choose your mood", text: "Pick a booth theme that matches the moment — romantic, cinematic, vintage, soft, or fun." },
  { number: "02", title: "Pose with the countdown", text: "Amoura guides the session like a real booth so every shot feels natural and exciting." },
  { number: "03", title: "Capture solo or together", text: "Use Single Mode for yourself or Dual Mode to connect with someone from anywhere." },
  { number: "04", title: "Keep the strip", text: "Get a beautiful photostrip that is ready to save, share, or look back on later." },
];

const AUDIENCES = ["Long-distance couples", "Date nights", "Best friends", "Solo portraits", "Birthdays", "Anniversaries", "Graduations", "Online celebrations"];

const FAQS = [
  { q: "What is Amoura?", a: "Amoura is a virtual photobooth PWA that lets people create beautiful photostrips using their phone or laptop camera." },
  { q: "Is Amoura only for couples?", a: "No. Amoura is perfect for couples, friends, solo portraits, barkada memories, celebrations, and online moments." },
  { q: "What is Dual Mode?", a: "Dual Mode lets two people join the same booth session remotely, follow one shared countdown, and create a connected photostrip experience." },
  { q: "Do users need to install an app?", a: "No. Amoura is built as a Progressive Web App, so users can open it in the browser and later add it to their home screen." },
];

// --- Optimized Animation Variants ---

const FADE_UP: Variants = {
  hidden: { opacity: 0, y: 20 }, // Reduced y distance for smoother feel
  visible: { opacity: 1, y: 0 },
};

// --- Memoized Sub-components for Speed ---

const TrustPill = memo(({ icon: Icon, text }: { icon: any; text: string }) => (
    ...
    <main className="amoura-page min-h-screen">
        ...
        <div className="amoura-container relative grid min-h-[calc(100vh-7rem)] items-center gap-12 pb-16 lg:grid-cols-[0.96fr_1.04fr] lg:gap-16">
          ...
          >
            ...
            <h1 className="amoura-serif text-5xl leading-[0.92] text-amoura-cream sm:text-6xl md:text-7xl xl:text-8xl">
              Feel close, even when you’re <span className="amoura-script">far apart.</span>
            </h1>

            <p className="mt-7 max-w-2xl text-base leading-8 text-amoura-muted sm:text-lg">
              Amoura turns your camera into a premium photobooth experience — with guided countdowns, cinematic themes, instant photostrips, and Dual Mode for shared memories from anywhere.
            </p>

            <div className="mt-9 flex flex-col gap-4 sm:flex-row">
              <Link href="/create-account" className="amoura-btn-primary inline-flex items-center justify-center gap-2 rounded-full px-8 py-4 text-base font-semibold">
                Create your first memory <ArrowRight className="h-5 w-5" />
              </Link>
              <a href="#dual-mode" className="amoura-btn-secondary inline-flex items-center justify-center gap-2 rounded-full px-8 py-4 text-base font-semibold">
          ...
          <div className="amoura-glass overflow-hidden rounded-[2rem] p-6 sm:p-10 lg:p-14">
            <div className="grid gap-12 lg:grid-cols-[0.92fr_1.08fr] lg:items-center">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={FADE_UP}>
                <p className="mb-4 text-sm font-semibold uppercase tracking-[0.35em] text-amoura-red-soft">Dual Mode</p>
                <h2 className="amoura-serif text-4xl leading-tight text-amoura-cream sm:text-6xl">One booth feeling, even from two places.</h2>
                <p className="mt-6 text-lg leading-8 text-amoura-muted">
                  Dual Mode is designed to make distance feel smaller. Two people join one session, follow one countdown, and create a connected photostrip that feels like it happened in the same booth.
                </p>
                ...
                <a href="#faq" className="amoura-btn-secondary mt-9 inline-flex items-center justify-center gap-2 rounded-full px-8 py-4 font-semibold">
            ...
            >
              <div className="relative flex aspect-[4/5] items-center justify-center overflow-hidden bg-gradient-to-br from-amoura-red/30 via-black to-amoura-red-deep/40">
                ...
                <div className="relative grid w-24 gap-2 rounded-xl border border-amoura-red-soft/30 bg-black/50 p-2">
                  {[1, 2, 3].map((i) => <div key={i} className="aspect-[4/3] rounded-md bg-amoura-cream/20" />)}
                  <p className="amoura-serif pt-2 text-center text-lg text-amoura-red-soft">Amoura</p>
                </div>
              </div>
              <div className="p-5">
      ...
      <section className="amoura-container py-20 sm:py-24">
        <div className="grid gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          <div>
            <p className="mb-4 text-sm font-semibold uppercase tracking-[0.35em] text-amoura-red-soft">Who it is for</p>
            <h2 className="amoura-serif text-4xl leading-tight text-amoura-cream sm:text-6xl">For moments people already want to remember.</h2>
            <p className="mt-6 text-lg leading-8 text-amoura-muted">
              Amoura works for romantic dates, solo portraits, online bonding, friend groups, birthdays, graduations, anniversaries, and small moments that become important later.
            </p>
          </div>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {AUDIENCES.map((item) => (
              <div key={item} className="rounded-2xl border border-amoura-red-soft/20 bg-white/[0.035] px-5 py-4 text-amoura-cream">
        ...
        <div className="amoura-card-highlight rounded-[2rem] p-8 sm:p-12">
          <div className="grid gap-10 lg:grid-cols-[1fr_0.85fr] lg:items-center">
            <div>
              ...
              <h2 className="amoura-serif text-4xl leading-tight text-amoura-cream sm:text-6xl">Amoura is not about taking more photos. It is about making one moment feel unforgettable.</h2>
            </div>
            <div className="space-y-4 text-lg leading-8 text-amoura-muted">
              <p>A selfie captures what you looked like. A photobooth captures how the moment felt.</p>
              <p>That is the promise of Amoura: open the booth and leave with something you want to save.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="faq" className="amoura-container py-20 sm:py-24">
        ...
        <div className="mx-auto mt-12 max-w-3xl space-y-4">
          {FAQS.map((faq) => (
            <details key={faq.q} className="group rounded-3xl border border-amoura-red-soft/20 bg-white/[0.035] p-6 transition-all">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-5 text-lg font-semibold text-amoura-cream">
                {faq.q} <span className="text-amoura-red-soft transition-transform duration-300 group-open:rotate-45">+</span>
              </summary>
      ...
      <section className="amoura-container pb-24 pt-8">
        <div className="relative overflow-hidden rounded-[2.25rem] border border-amoura-red-soft/30 bg-gradient-to-br from-amoura-red-deep/50 via-amoura-black-soft to-black p-8 text-center shadow-[0_0_90px_rgba(194,31,58,0.22)] sm:p-14">
          ...
          <div className="relative">
            ...
            <h2 className="amoura-serif mx-auto max-w-4xl text-4xl leading-tight text-amoura-cream sm:text-6xl">Your next memory deserves more than a screenshot.</h2>
            <p className="mx-auto mt-6 max-w-2xl text-lg leading-8 text-amoura-muted">
              Create your account and be ready for the virtual photobooth made for love, distance, friendship, and unforgettable little moments.
            </p>
            <Link href="/create-account" className="amoura-btn-primary mt-9 inline-flex items-center justify-center gap-2 rounded-full px-9 py-4 font-semibold">
              Create your first memory <ArrowRight className="h-5 w-5" />
            </Link>
          </div>
        </div>
...
const Header = memo(({ open, setOpen }: { open: boolean; setOpen: (open: boolean) => void }) => {
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/5 bg-black/60 backdrop-blur-xl">
      <nav className="amoura-container flex h-20 items-center justify-between">
        <Link href="/" className="leading-none group">
          <p className="amoura-serif text-3xl text-amoura-red-soft transition-colors group-hover:text-amoura-pink">Amoura</p>
          <p className="text-[0.62rem] font-semibold uppercase tracking-[0.35em] text-amoura-muted">Virtual Photobooth</p>
        </Link>
        ...
        <div className="hidden lg:block">
          <Link href="/create-account" className="amoura-btn-primary rounded-full px-5 py-3 text-sm font-semibold">Create Account</Link>
        </div>
        <button onClick={() => setOpen(!open)} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-amoura-red-soft/25 text-amoura-cream lg:hidden" aria-label="Toggle menu">
          {open ? <X /> : <Menu />}
        </button>
      </nav>
      {open && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="border-t border-white/5 bg-black/95 px-6 py-5 lg:hidden flex flex-col gap-4">
          {NAV_ITEMS.map((item) => (
            <a key={item.href} href={item.href} onClick={() => setOpen(false)} className="py-2 text-amoura-muted hover:text-white">{item.label}</a>
          ))}
          <Link href="/create-account" className="amoura-btn-primary mt-2 rounded-full px-5 py-3 text-center text-sm font-semibold">Create Account</Link>
        </motion.div>
      )}
    </header>
  );
  ...
  >
    ...
    <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }} className="relative">
      <Image
        ...
        height={900}
        priority
        className="relative z-10 w-full h-auto object-contain drop-shadow-[0_0_55px_rgba(194,31,58,0.35)]"
      />
    </motion.div>
    ...
    <div className="rounded-[2rem] border border-amoura-red-soft/30 bg-black/40 p-4 shadow-[0_40px_120px_rgba(0,0,0,0.45)]">
      <div className="relative overflow-hidden rounded-[1.5rem] border border-white/10 bg-gradient-to-br from-black via-[#17070b] to-black p-4">
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="rounded-full bg-amoura-red/20 px-3 py-1 text-xs text-amoura-pink">Synced</span>
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="relative flex aspect-[4/5] items-center justify-center overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04]">
            <Avatar />
            <span className="absolute bottom-3 left-3 rounded-full bg-black/50 px-3 py-1 text-xs text-amoura-muted">You</span>
          </div>
          <div className="relative flex aspect-[4/5] items-center justify-center overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04]">
            <Avatar delay />
            <span className="absolute bottom-3 left-3 rounded-full bg-black/50 px-3 py-1 text-xs text-amoura-muted">Them</span>
          </div>
        </div>
...
const Avatar = memo(({ delay = false }: { delay?: boolean }) => (
  <motion.div animate={{ y: delay ? [0, -5, 0] : [0, 5, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }} className="flex flex-col items-center">
...
const SectionIntro = memo(({ eyebrow, title, text }: { eyebrow: string; title: string; text: string }) => (
  <div className="mx-auto max-w-3xl text-center">
    <p className="mb-4 text-sm font-semibold uppercase tracking-[0.35em] text-amoura-red-soft">{eyebrow}</p>
    <h2 className="amoura-serif text-4xl leading-tight text-amoura-cream sm:text-6xl">{title}</h2>
